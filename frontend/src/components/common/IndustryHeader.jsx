...
export default function IndustryHeader({
  ...
  const pageTitle = title || t('industry_portal', 'Industry & CSR Dashboard');
  const pageSubtitle = subtitle || 'Manage CSR allocations, university partnerships, and innovative solutions.';
  const pageBadge = badgeText || t('industry_portal', 'Industry Portal');
  const pagePlaceholder = searchPlaceholder || t('search_placeholder', 'Search CSR projects, challenges, university proposals...');
  ...
  const userName = currentUser?.name || currentUser?.company || 'Industry Partner';
  const userInitials = (userName || 'IP').split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase();
  const userOrg = currentUser?.organization_or_district || currentUser?.company || 'Industry CSR';

  const fetchLiveNotifications = async () => {
    try {
      const res = await notificationApi.getNotifications({ role: 'industry' });
      if (res && res.success && res.notifications) {
        setNotifications(res.notifications);
        setUnreadCount(res.unreadCount || 0);
      }
    } catch (err) {
      console.warn('Error fetching industry notifications:', err);
    }
  };
  ...
  const markAllRead = async () => {
    try {
      await notificationApi.markAllAsRead('industry');
      setUnreadCount(0);
  ...
  const handleNotificationClick = async (notif) => {
    ...
    if (notif.type === 'COLLABORATION') {
      ...
      navigate('/industry/communication');
    } else {
    ...
    <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 pb-3 border-b border-slate-200/80 sticky top-0 z-20 bg-[#f4f5fa]/95 backdrop-blur-sm pt-1">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {subtitle && (
            <p className="text-xs sm:text-[13px] text-slate-500 mt-0.5">{subtitle}</p>
          )}
        </div>
      </div>

      {/* Right Tools: Action buttons, Search, Notification Bell & Interactive Profile Popover */}
      <div className="flex items-center gap-3 self-start md:self-auto flex-wrap">
        ...
        {onSearch && (
          <div className="relative hidden sm:block w-48 lg:w-60">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-emerald-600 focus:border-emerald-600 shadow-2xs"
            />
          </div>
        )}
        ...
        <div className="relative">
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[16px] h-[16px] px-1 bg-amber-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center">
                {unreadCount}
              </span>
            )}
          </button>
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-80 sm:w-96 max-w-[calc(100vw-2rem)] bg-white rounded-2xl shadow-xl border border-slate-200 z-50 overflow-hidden text-left animate-in fade-in">
              <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/70">
                <div>
                  <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">CSR &amp; Partnership Notices</h3>
              ...
              <div className="overflow-y-auto max-h-[340px] divide-y divide-slate-100 text-xs">
                {notifications.length === 0 ? (
                  ...
                  notifications.map((notif) => (
                    ...
                    >
                      <div className={`mt-1.5 w-2 h-2 rounded-full shrink-0 ${!notif.is_read ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`} />
        ...
        {liveToast && (
          <div className="fixed top-4 right-4 z-50 bg-[#0b1329] text-white p-3.5 rounded-xl shadow-2xl border border-emerald-500/40 flex items-start gap-3 max-w-sm animate-bounce">
            ...
            <div className="flex-1 text-xs">
              <p className="font-bold text-emerald-300">{liveToast.title}</p>
        ...
        <div className="relative">
          <div
            ...
            className="flex items-center gap-3 bg-white px-3.5 py-1.5 rounded-lg border border-slate-200 shadow-xs cursor-pointer hover:border-emerald-400 transition"
          >
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-emerald-600 to-teal-700 text-white font-bold flex items-center justify-center text-xs shadow-xs">
              {userInitials}
            </div>
          ...
          {showProfileDropdown && (
            <div className="absolute right-0 mt-2 w-72 bg-white rounded-2xl shadow-xl border border-slate-200 z-50 overflow-hidden animate-in fade-in">
              <div className="p-4 bg-gradient-to-br from-[#0d1927] to-[#09121d] text-white">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-emerald-500 to-teal-400 flex items-center justify-center text-white font-bold text-sm shadow-md border border-white/20">
                    {userInitials}
                  </div>
                  <div>
